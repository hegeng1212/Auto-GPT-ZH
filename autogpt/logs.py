...
class Logger(metaclass=Singleton):
    ...
    def double_check(self, additionalText=None):
        if not additionalText:
            additionalText = (
                "请确保您已经设置并配置了所有内容"
                "可查看群文档解决常见报错问题https://nhrvt0kw31.feishu.cn/docx/FDEUd0YhKolp52xFPrMc34e9nse"
                "或者去群里问一下"
            )

        self.typewriter_log("双重检查配置", Fore.YELLOW, additionalText)
...
def print_assistant_thoughts(ai_name, assistant_reply):
    ...
    try:
        ...
        logger.typewriter_log(
            "推理:", Fore.YELLOW, f"{assistant_thoughts_reasoning}"
        )
        ...
        logger.typewriter_log(
            "意见:", Fore.YELLOW, f"{assistant_thoughts_criticism}"
        )
...
def print_assistant_thoughts(
    ...
    if assistant_thoughts_plan:
        ...
        for line in lines:
            line = line.lstrip("- ")
            logger.typewriter_log("- ", Fore.GREEN, line.strip())
    logger.typewriter_log("评价:", Fore.YELLOW, f"{assistant_thoughts_criticism}")
    # Speak the assistant's thoughts
    if CFG.speak_mode and assistant_thoughts_speak:
        say_text(assistant_thoughts_speak)
